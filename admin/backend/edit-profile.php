...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
        ...
        if ($confirm_password === $new_password) {
            ...
            $sql = "UPDATE `user` SET `userName`='$fullname',`email`='$email',`password`='$hash'WHERE userID = '$userID' ";

            try {
                mysqli_query($conn, $sql);
                $status = "1";
                header("Location:/meetbook/admin/profile-edit.php?id=$userID&status=$status");
            } catch (mysqli_sql_exception) {
                $status = "2";
                header("Location:/meetbook/admin/profile-edit.php?id=$userID&status=$status");
            }
        ...
        // UPDATE USER
        $sql = "UPDATE `user` SET `userName`='$fullname',`email`='$email'WHERE userID = '$userID' ";
